...
import {
    ...
    getActiveDims, getActiveModelId, isModelReady, isJobRunning, embedText,
...
export async function updateSceneAssistant(campaignId, sceneIdParam, assistantContent) {
    ...
    if (isModelReady()) {
        try {
            const text = buildArchiveText(newIndexEntry);
            const embedding = await embedText(text);
            await storeArchiveEmbedding(campaignId, targetId, embedding);
        } catch (e) {
            console.warn('[Archive Edit-Sync] Failed to re-embed updated scene:', e.message);
        }
    }
