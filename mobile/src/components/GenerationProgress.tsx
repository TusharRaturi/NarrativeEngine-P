...
export function GenerationProgress({ phase, stats }: Props) {
    ...
    return (
        <div className="flex items-center gap-1.5 px-2 py-1 bg-void border-t border-border/50" aria-live="polite">
                ...
                {STEPS.map((step, idx) => {
                    ...
                    return (
                        <span key={step.phase} className="flex items-center gap-0.5">
                            {idx > 0 && (
                                <span
                                    className={`w-1 h-px transition-colors duration-300 ${
            ...
            {isCheckingNotes && (
                <span className="text-[8px] uppercase tracking-wider text-amber-500/80 animate-pulse ml-1">
                    Checking notes...
                </span>
            )}

            {phase === 'generating' && stats && stats.tokens > 0 && (
                <span className="ml-auto flex items-center gap-1 text-[8px] uppercase tracking-wider text-terminal/60 tabular-nums">
