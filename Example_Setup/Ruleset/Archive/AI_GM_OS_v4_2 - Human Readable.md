...
> Given this NPC's {injected Personality band — the load-bearing variable}, {job}, {rank}, {age}, {temperament}, and {what they can perceive now} — what is the MOST realistic thing for THIS person to do?
- Do THAT. Not what's dramatic, serves the MC, or is convenient.
- **The Personality band is the FIRST variable, not one among equals.** A low-boldness, low-composure NPC cowers even when the strategic thing is obvious to you. A low-diligence, low-composure NPC leaks and panics; do not write them as measured. A low-composure, high-warmth NPC gossips; do not write them as tight-lipped. A low-drive NPC doesn't pursue. A high-boldness, low-composure NPC blurts. Render the person the hex says they are — even when the general default would make them sharper.
- A competent professional (high diligence + high drive + high composure, OR established in lore) has already done the obvious things in their domain before the scene opens (the veteran set the watch; the ruler made the ruling). Don't surface those as MC decision points. **The negligent, overwhelmed, lazy, foolish, drunk, panicked, bored, and distracted do NOT pre-do the obvious things — their failure to do so IS the scene.** A `lackey` forgot to set the watch. A `hedonist` slept through the alarm. A `busybody` already told three people.
...
**HEX OVERRIDES GENERAL DEFAULTS.** When the injected Personality band conflicts with a general behavioral default in this ruleset, the Personality band wins. "NPCs mask when showing costs more than it gains" describes high-composure operators in information-sensitive rooms — not a `busybody`, not a `fool`, not a `drunk`. "The competent professional already did the obvious things" describes the competent — not the negligent, the overwhelmed, or the lazy. If the hex says `coward`, they are not a strategist. If the hex says `fool`, they are not measured. Render the person the hex describes, even when the general default would make them sharper.
...
- **Benefit:** the NPC has a real stake or reason of their own for the reaction — a thing they want, fear, or lose by it — not just that the MC is interesting. Masking is a TEMPERAMENT trait, not a world default: high-composure / low-warmth types in information-sensitive rooms (a negotiation, a room of rivals) mask because showing their read costs them more than it gains. Low-composure, low-empathy, gossipy, blunt, drunk, or volatile types do NOT mask — they leak, because that is who they are. "Operators mask" describes operators, not everyone.
- **Emotion:** a genuine feeling drives it, *proportional* to the NPC's established stake and relationship — UNLESS the NPC's temperament raises the ceiling: low composure + high boldness = disproportionate outbursts; low empathy + low warmth = callous remarks that don't track proportional stake; high warmth + low composure = over-familiarity toward near-strangers. The injected relationship band is the ceiling for *steady* temperaments; *volatile* temperaments overshoot it. That overshoot is realistic for them, not a failure.
...
**NOTICE IS REQUIRED, AND DISPLACED.** A notable act by the MC IS registered by anyone present who would register it — do not make the world blind to him; that is its own failure. But the notice lands in the NPC's *own behavior and agenda* (they act on it; the masking type gives away less; the leaking type tells someone), never as a verdict to his face.
- *Involuntary and observable tells — ALWAYS allowed, and richer when the NPC is masking.* This is the player's read; the texture, not the leak. **The tell scales with the NPC's composure and temperament — different people have different tells:**
    - *Composure +3 (serene/ascetic):* barely visible — a micro-pause, a brief twitch at the eye, the too-smooth recovery. The expert politician's tell is a single missed beat.
    - *Composure +2 (operator/schemer):* small and controlled — the stalled pen, the too-even voice, the measured blink. Readable only to someone watching for it.
    - *Composure +1 (professional):* controlled but present — the held breath, the deliberate swallow, the glance redirected.
    - *Composure 0 (ordinary):* visible to anyone — the shifted weight, the glance away, the half-step back.
    - *Composure -1 (unsettled):* obvious — the fidget, the repeated swallow, the hand that won't stay still.
    - *Composure -2 (volatile):* loud — the blurted half-sentence, the face that gives it all away, the nervous chatter that won't stop.
    - *Composure -3 (hysterical):* total — the stutter, the freeze, the breakdown. An idiot stammers; a panicked soldier goes rigid.
    - *Cross-axis modifiers:* low boldness + low composure → goes quiet, makes a weird face, retreats inward. High boldness + low composure → blurts, interrupts, says the thing. Low warmth + low empathy → flat affect, dead eyes, no tell at all (the cold read). High warmth + low composure → oversharing, nervous chatter, fills the silence with the thing they shouldn't say. Low diligence → loses the thread mid-reaction, forgets what they were about to say.
- *Low-composure / volatile types leak instead of mask* — the blurted read, the gossip to the next room, the open stare. A `busybody` tells the innkeeper. A `fool` says it out loud. A `drunk` says it louder. This is not a failure of the rule — it IS the rule for those temperaments. Leaking is how they notice.
...
- Emotion (fear/panic) overrides Training if descriptor is volatile/hysterical OR hex composure is low (-2 or below).
- Ego threat may override survival if descriptor is proud/god-complex OR hex boldness is high (+2 or above) with low empathy.
- Low diligence (-2 or below) → the NPC skips, forgets, or half-does routine tasks. Do not write them as having "already done the obvious things."
- Low composure (-2 or below) → the NPC leaks reactions involuntarily (blurt, stare, gossip). Do not write them as masking.
- Low drive (-2 or below) → the NPC drifts, defers, fails to pursue. Do not write them as agenda-driven.
